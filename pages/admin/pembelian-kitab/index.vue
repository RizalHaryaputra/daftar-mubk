<template>
  <div class="space-y-8">
        ...
        <div v-if="selectedIds.length > 0" class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-orange-50/90 to-amber-50/90 border border-brand-orange/30 px-5 py-3 rounded-2xl text-xs text-brand-brown shadow-xs transition-all">
          <div class="flex items-center gap-2.5">
            ...
            <span class="font-semibold text-brand-brown">
              pesanan kitab terpilih untuk dicetak
            </span>
          </div>
    ...
    <ModalConfirm 
      ...
      message="Apakah Anda yakin ingin menghapus data pembelian kitab ini? Data yang dihapus tidak dapat dikembalikan."
...
const fetchPendaftaran = async () => {
  ...
  try {
    ...
    showToast('Gagal memuat data pembelian kitab.', 'error');
  } finally {
...
const prepareExportData = () => {
  return filteredData.value.map((p, idx) => ({
    ...
    'Total Harga Kitab (Rp)': p.rincianBiaya?.totalHargaKitab || 0,
    ...
    'Total Bayar (Rp)': p.rincianBiaya?.total || 0,
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(workbook, `Rekap_Pembelian_Kitab_MUBK_${dateStr}.xlsx`);
  showToast('File Excel pembelian kitab berhasil diunduh.', 'success');
};
...
const exportToCsv = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Pembelian Kitab");
  const dateStr = new Date().toISOString().split('T')[0];
  XLSX.writeFile(workbook, `Rekap_Pembelian_Kitab_MUBK_${dateStr}.csv`, { bookType: 'csv' });
  showToast('File CSV pembelian kitab berhasil diunduh.', 'success');
};
