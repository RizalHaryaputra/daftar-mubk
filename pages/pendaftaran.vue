<template>
  <div class="max-w-4xl mx-auto space-y-8 pb-20 pt-8 px-4">
    ...
    <template v-else>
      ...
      <div v-if="selectedProgram" class="bg-brand-brown text-white rounded-[30px] p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl">
        <div>
      ...
      <form @submit.prevent="submitForm" class="space-y-10">
            ...
            <div class="flex flex-col gap-3">
              ...
              <div class="flex gap-8">
                <label class="flex items-center gap-3 text-sm cursor-pointer group">
                  <input type="radio" v-model="form.dataPeserta.jenisKelamin" value="laki-laki" required class="w-5 h-5 text-brand-orange focus:ring-brand-orange border-brand-border" />
                  <span class="group-hover:text-brand-orange transition-colors">Laki-laki</span>
                </label>
                <label class="flex items-center gap-3 text-sm cursor-pointer group">
                  <input type="radio" v-model="form.dataPeserta.jenisKelamin" value="perempuan" class="w-5 h-5 text-brand-orange focus:ring-brand-orange border-brand-border" />
        ...
        <div v-if="semuaKitabDibeli.length > 0" class="bg-brand-deeper text-white border border-brand-deeper rounded-[30px] p-8 md:p-10 shadow-xl relative overflow-hidden">
          ...
          <div class="space-y-6 relative z-10">
            ...
            <div class="flex flex-col gap-2">
              ...
              <select v-model="form.ongkir.zona" required class="px-5 py-4 text-sm rounded-xl border border-white/20 bg-white/5 focus:outline-none focus:border-brand-orange transition-colors w-full text-white appearance-none">
        ...
        <!-- Error Submission -->
        <div v-if="submitError" class="bg-red-50 text-red-600 p-6 rounded-[20px] text-center border border-red-200">
          <p class="font-bold mb-1">Gagal Memproses Data</p>
          {{ submitError }}
        </div>

        <button type="submit" :disabled="isSubmitting" class="w-full bg-brand-orange text-white font-display text-xl md:text-2xl py-6 rounded-full hover:bg-orange-600 transition-all hover:shadow-xl hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none">
