  ...
  <div class="space-y-12 pb-20 pt-8">
    ...
    <div class="gsap-header text-center max-w-2xl mx-auto px-4">
      <h1 class="font-display text-4xl md:text-5xl text-brand-brown mb-4 font-bold tracking-tight">
        Kitab <span class="text-brand-orange italic">Rujukan</span>
      </h1>
      <p class="text-brand-muted md:text-lg leading-relaxed">
        Koleksi kitab rujukan otentik yang digunakan sebagai kurikulum utama 
        di Ma'had 'Umar bin Khattab.
      </p>
    </div>
    ...
    <div class="gsap-search max-w-md mx-auto px-4">
      <div class="relative">
        ...
        <input 
          ...
          v-model="searchQuery" 
          @input="currentPage = 1"
          placeholder="Cari kitab rujukan..." 
    ...
    <div v-else-if="error" class="bg-red-50 text-red-600 p-6 rounded-2xl text-center border border-red-100 max-w-md mx-auto">
      <p class="font-medium">Ups, gagal memuat data kitab.</p>
      <p class="text-sm mt-1 opacity-80">Silakan periksa koneksi Anda dan coba muat ulang halaman.</p>
    </div>

    <!-- State Data -->
    <div v-else-if="filteredKitabs.length > 0" class="max-w-6xl mx-auto px-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
        <div v-for="kitab in paginatedKitabs" :key="kitab.id" class="gsap-kitab-card">
          <CardKitab :kitab="kitab" />
        </div>
      </div>
    ...
    <div v-else class="text-center py-20 bg-white/50 rounded-[40px] max-w-3xl mx-auto border border-brand-border border-dashed">
      ...
      <p class="text-xl font-display text-brand-brown mb-2">{{ searchQuery ? 'Kitab tidak ditemukan' : 'Belum ada koleksi kitab' }}</p>
      <p class="text-brand-muted">{{ searchQuery ? `Tidak ada kitab yang cocok dengan pencarian "${searchQuery}"` : 'Koleksi kitab sedang dipersiapkan, silakan kembali lagi nanti.' }}</p>
    </div>
  </div>
</template>
...
const filteredKitabs = computed(() => {
  if (!searchQuery.value.trim()) return kitabs.value;
  const q = searchQuery.value.toLowerCase();
  return kitabs.value.filter(k => 
...
const totalPages = computed(() => {
  return Math.ceil(filteredKitabs.value.length / itemsPerPage.value);
});

const paginatedKitabs = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  const end = start + itemsPerPage.value;
  return filteredKitabs.value.slice(start, end);
});
...
const animateCards = async () => {
  if (!import.meta.client) return;
  await nextTick();
  const cards = document.querySelectorAll('.gsap-kitab-card');
  if (cards.length > 0) {
...
onMounted(async () => {
  ...
  try {
    ...
    let allKitabs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    allKitabs.sort((a: any, b: any) => {
      const dateA = a.createdAt?.toDate ? a.createdAt.toDate() : new Date(a.createdAt || 0);
      const dateB = b.createdAt?.toDate ? b.createdAt.toDate() : new Date(b.createdAt || 0);
      return dateB.getTime() - dateA.getTime();
    });
    kitabs.value = allKitabs;
  } catch (e) {
